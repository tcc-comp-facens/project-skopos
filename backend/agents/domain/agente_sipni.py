"""
Agente de Domínio — SI-PNI (Sistema de Informação do Programa Nacional
de Imunizações).

Especializado em cobertura vacinal e doses aplicadas. Consulta
`IndicadorSaude(sistema="sipni")` no Neo4j via `neo4j_client`.

Substitui `AgenteAtencaoPrimaria` (migração mínima da Fase 1, que
relabelava "cobertura_vacinal" para o tipo legado "vacinacao") — decisão
de particionamento em PLANO_NOVO_MODELO_DADOS.md §5 (1 agente de saúde
por Sistema de Informação). Este agente cobre os dois subtipos nativos
do SI-PNI (`cobertura_vacinal`, `doses_aplicadas`) sem relabeling — o
vocabulário do chat (`agente_interpretacao_intencao.py`) é estendido
separadamente para reconhecer ambos.

Replica o padrão de referência de `agente_sinan.py` (Fase 2).
"""
...
SISTEMA = "sipni"
SUBTIPOS: list[str] = ["cobertura_vacinal", "doses_aplicadas"]


class AgenteSIPNI(AgenteCoALA):
    """Agente de domínio especializado no SI-PNI — cobertura vacinal.
    ...
    (`COBERTURA_VACINAL`, `DOSES_APLICADAS`) + "sem quebra";
    `evaluate_and_select` arbitra entre eles via score determinístico +
    LLM opcional (`agents.domain.query_planning.arbitrar_dimensao`).
    ...
    def _act_consultar_indicadores(self, action: dict) -> None:
        """Ação externa (grounding): consulta `IndicadorSaude(sistema="sipni")`
        ...
        try:
            logger.info(
                "Agent %s: consultando indicadores (sistema=sipni, subtipos=%s, "
    ...
    def query(
        ...
        """Consulta indicadores SI-PNI (cobertura vacinal, doses
        aplicadas), sem despesas.
