...
class AgenteSaudeHospitalar(AgenteBDI):
    ...
    perceive → deliberate → plan → execute (Req 2.4).
    ...
    def _execute_intention(self, intention: dict) -> None:
        ...
        Para "consultar_despesas": busca DespesaSIOPS com subfuncao=302,
        filtrando apenas os registros da subfunção 302 (Req 2.2).

        Para "consultar_indicadores": busca IndicadorDataSUS com
        tipo="internacoes" (Req 2.1).
        ...
        try:
            if goal == "consultar_despesas":
                ...
                # Filtra apenas subfunção 302 — Assistência Hospitalar (Req 2.2)
                despesas = [
    ...
    def _recover_intention(self, failed_intention: dict) -> dict | None:
        """Recuperação de falha: retorna listas vazias (Req 2.5).
