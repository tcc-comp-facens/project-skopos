"""
Agente de Domínio — COVID-19.

Especializado em casos e óbitos de COVID-19. Consulta
`IndicadorSaude(sistema="covid")` no Neo4j via `neo4j_client`, cobrindo
os dois subtipos nativos (`casos`, `obitos`).

Substitui a metade "covid" de `AgenteVigilanciaEpidemiologica` (migração
mínima da Fase 1, que só consultava `casos` e nunca `obitos`) — decisão
de particionamento em PLANO_NOVO_MODELO_DADOS.md §5 (1 agente de saúde
por Sistema de Informação). `sistema="covid"` não tem dimensões válidas
(`SISTEMA_DIMENSOES["covid"] == []`), então este agente não delibera
entre candidatos — sempre consulta sem quebra dimensional, mesma
justificativa documentada em `AgenteOrcamentoSubfuncao`.

Replica o padrão de referência de `agente_sinan.py` (Fase 2), sem a
deliberação de dimensão (não aplicável aqui).
"""
...
SISTEMA = "covid"
SUBTIPOS: list[str] = ["casos", "obitos"]


class AgenteCOVID(AgenteCoALA):
    """Agente de domínio especializado em COVID-19 — casos e óbitos.

    `sistema="covid"` não tem dimensões válidas no schema atual — não há
    ...
    def _act_consultar_indicadores(self, action: dict) -> None:
        """Ação externa (grounding): consulta `IndicadorSaude(sistema="covid")`
        no Neo4j, cobrindo `casos` e `obitos`.
        ...
        try:
            logger.info(
                "Agent %s: consultando indicadores (sistema=covid, subtipos=%s, "
                "periodo=%s-%s)",
    ...
    def query(
        ...
        """Consulta indicadores COVID-19 (casos, óbitos), sem despesas.
        ...
        Args:
            ...
            intent_summary: Resumo da intenção do usuário (não usado —
                ...
                agentes de saúde, chamados uniformemente pelo
