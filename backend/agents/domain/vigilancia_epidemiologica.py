...
class AgenteVigilanciaEpidemiologica(AgenteCoALA):
    ...
    perceive → propose_actions → evaluate_and_select → execute (Req 1.4).
    ...
    def propose_actions(self) -> list[dict]:
        ...
        consulta e então consultar despesas (subfunção 305) e
        indicadores (dengue, covid).
    ...
    def _act_consultar_despesas(self, action: dict) -> None:
        ...
        Filtra pelas subfunções do plano de consulta (Req 1.2), resolvido
    ...
    def _act_consultar_indicadores(self, action: dict) -> None:
        ...
        Busca os tipos definidos no plano de consulta (Req 1.1),
    ...
    def _act_fallback_despesas(self, action: dict) -> None:
        """Estratégia de fallback (Req 1.5): grava lista vazia em working memory.
