"""
Agente de Domínio — SINASC (Sistema de Informações sobre Nascidos Vivos).

Especializado em dados de nascidos vivos. Consulta
`IndicadorSaude(sistema="sinasc")` no Neo4j via `neo4j_client`.
...
saúde por Sistema de Informação).

Replica o padrão de referência de `agente_sinan.py`.
"""
...
SISTEMA = "sinasc"
SUBTIPOS: list[str] = ["nascidos_vivos"]


class AgenteSINASC(AgenteCoALA):
    """Agente de domínio especializado no SINASC — nascidos vivos.
    ...
    (`POR_FAIXA_ETARIA`, `POR_FAIXA_PESO`) + "sem quebra";
    ...
    def _act_consultar_indicadores(self, action: dict) -> None:
        """Ação externa (grounding): consulta `IndicadorSaude(sistema="sinasc")`
        ...
        try:
            logger.info(
                "Agent %s: consultando indicadores (sistema=sinasc, subtipos=%s, "
    ...
    def query(
        ...
        """Consulta indicadores SINASC (nascidos vivos), sem despesas.
        ...
        Args:
            ...
            health_params: Indicadores solicitados na análise (não usado
                ...
                ativado, cobre todo o seu domínio).
