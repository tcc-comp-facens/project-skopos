"""
Agente de Domínio — SIM (Sistema de Informações sobre Mortalidade).

Especializado em dados de mortalidade. Consulta
`IndicadorSaude(sistema="sim")` no Neo4j via `neo4j_client`.

Substitui `AgenteMortalidade` (migração mínima da Fase 1) — decisão de
particionamento em PLANO_NOVO_MODELO_DADOS.md §5 (1 agente de saúde por
Sistema de Informação). Diferente do legado, que retornava despesas de
TODAS as subfunções (visão transversal), este agente NÃO consulta
despesas — orçamento é responsabilidade de `AgenteOrcamentoSubfuncao`,
consultado separadamente pelo orquestrador/supervisor.
...
SISTEMA = "sim"
SUBTIPOS: list[str] = ["mortalidade"]


class AgenteSIM(AgenteCoALA):
    """Agente de domínio especializado no SIM — mortalidade.
    ...
    (`POR_FAIXA_ETARIA`, `POR_CAPITULO_CID`) + "sem quebra";
    ...
    def _act_consultar_indicadores(self, action: dict) -> None:
        """Ação externa (grounding): consulta `IndicadorSaude(sistema="sim")`
        ...
        try:
            logger.info(
                "Agent %s: consultando indicadores (sistema=sim, subtipos=%s, "
    ...
    def query(
        ...
        """Consulta indicadores SIM (mortalidade), sem despesas.
