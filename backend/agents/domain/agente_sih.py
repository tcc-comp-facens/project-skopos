"""
Agente de Domínio — SIH (Sistema de Informações Hospitalares).

Especializado em internações hospitalares. Consulta
`IndicadorSaude(sistema="sih")` no Neo4j via `neo4j_client`.

Substitui `AgenteSaudeHospitalar` (migração mínima da Fase 1) — decisão
de particionamento em PLANO_NOVO_MODELO_DADOS.md §5 (1 agente de saúde
por Sistema de Informação).

Segundo agente a replicar o padrão de referência de `agente_sinan.py`
(Fase 2) — caso mais simples (1 subtipo, 1 dimensão), usado para
confirmar que o padrão generaliza fora do caso "9 subtipos".
...
SISTEMA = "sih"
SUBTIPOS: list[str] = ["internacoes"]


class AgenteSIH(AgenteCoALA):
    """Agente de domínio especializado no SIH — internações hospitalares.
    ...
    (`POR_CAPITULO_CID`) + "sem quebra"; `evaluate_and_select` arbitra
    entre eles via score determinístico + LLM opcional
    ...
    def _act_consultar_indicadores(self, action: dict) -> None:
        """Ação externa (grounding): consulta `IndicadorSaude(sistema="sih")`
        ...
        try:
            logger.info(
                "Agent %s: consultando indicadores (sistema=sih, subtipos=%s, "
    ...
    def query(
        ...
        """Consulta indicadores SIH (internações), sem despesas.
