"""
Unit tests for AgenteAtencaoPrimaria.
...
from agents.domain.atencao_primaria import (
...
class TestInheritanceAndInit:
    """Req 3.4: Inherits from AgenteBDI and implements BDI cycle."""

    def test_inherits_agente_bdi(self):
        client = MagicMock()
        agent = AgenteAtencaoPrimaria("ap-test-1", client)
    ...
    def test_agent_id_set(self):
        client = MagicMock()
        agent = AgenteAtencaoPrimaria("ap-test-1", client)
        assert agent.agent_id == "ap-test-1"
...
class TestBDICycle:
    """Req 3.4: Full BDI cycle (perceive, deliberate, plan, execute)."""
...
class TestQuery:
    """Req 3.1, 3.2, 3.3: Query filters by subfuncao=301 and tipo="vacinacao"."""

    def test_query_filters_despesas_subfuncao_301(self):
        """Req 3.2: Only subfuncao 301 despesas are returned."""
        client = MagicMock()
        ...
        assert result["despesas"][0]["subfuncao"] == 301

    def test_query_returns_vacinacao_indicadores(self):
        """Req 3.1: Queries IndicadorDataSUS tipo="vacinacao"."""
        client = MagicMock()
        client.get_despesas.return_value = []
        client.get_indicadores.return_value = [
            {"tipo": "vacinacao", "ano": 2020, "valor": 85.0},
    ...
    def test_query_returns_required_structure(self):
        """Req 3.3: Returns dict with despesas and indicadores keys."""
        client = MagicMock()
        client.get_despesas.return_value = [
            {"subfuncao": 301, "subfuncaoNome": "Atenção Básica", "ano": 2020, "valor": 100.0},
        ]
...
class TestEmptyData:
    """Req 3.5: Returns empty lists without exception when no data found."""
    ...
    def test_no_matching_subfuncao_returns_empty_despesas(self):
        """When Neo4j returns despesas but none match subfuncao 301."""
        client = MagicMock()
        client.get_despesas.return_value = [
            {"subfuncao": 302, "subfuncaoNome": "Assistência Hospitalar", "ano": 2020, "valor": 300.0},
            {"subfuncao": 305, "subfuncaoNome": "Vigilância Epidemiológica", "ano": 2020, "valor": 200.0},
    ...
    def test_neo4j_failure_returns_empty_lists(self):
        """Req 3.5: Graceful degradation on Neo4j failure."""
        client = MagicMock()
...
class TestDomainConstants:
    ...
    def test_subfuncao_is_301(self):
        assert SUBFUNCAO == 301

    def test_tipos_indicador(self):
        assert set(TIPOS_INDICADOR) == {"vacinacao"}
