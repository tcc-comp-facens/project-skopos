"""Tests for AgenteSINAN — agente de referência de saúde da Fase 1.

Cobre a deliberação real de dimensão (propose_actions/evaluate_and_select
com candidatos concorrentes de verdade — PLANO_NOVO_MODELO_DADOS.md §5) e
a consulta a IndicadorSaude(sistema="sinan") via neo4j_client.
"""
...
from agents.domain.agente_sinan import SISTEMA, SUBTIPOS, AgenteSINAN
...
def agente(neo4j_client):
    return AgenteSINAN("test-sinan", neo4j_client)


class TestSemanticMemory:
    def test_sistema_is_sinan(self, agente):
        assert agente.semantic_memory["sistema"] == "sinan"

    def test_subtipos_has_9_diseases(self, agente):
        assert len(agente.semantic_memory["subtipos"]) == 9
        assert "dengue" in agente.semantic_memory["subtipos"]

    def test_dimensoes_validas_from_query_builder(self, agente):
        assert agente.semantic_memory["dimensoes_validas"] == ["POR_FAIXA_ETARIA", "POR_SEXO"]
...
class TestProposeActionsGeraCandidatosReais:
    def test_proposes_one_candidate_per_dimension_plus_no_break(self, agente):
        ...
        assert dimensoes == {None, "POR_FAIXA_ETARIA", "POR_SEXO"}
...
class TestEvaluateAndSelectSemLlm:
    def test_use_llm_false_picks_highest_score(self, agente):
        agente.update_working_memory({
            "analysis_id": "a1", "date_from": 2019, "date_to": 2021,
            "use_llm": False, "intent_summary": "quero saber a dengue por faixa etária",
        })
        candidates = agente.propose_actions()
        selected = agente.evaluate_and_select(candidates)
        assert len(selected) == 1
        # "faixa etaria" está no intent_summary — score 1.0 vs 0.5 do "sem quebra"
    ...
    def test_use_llm_false_no_mention_falls_back_to_no_break(self, agente):
        ...
        candidates = agente.propose_actions()
        selected = agente.evaluate_and_select(candidates)
        # Nenhuma dimensão mencionada — "sem quebra" (score 0.5) vence por padrão
...
class TestEvaluateAndSelectComLlm:
    def test_llm_choice_is_respected(self, agente):
        agente.update_working_memory({
            "analysis_id": "a1", "date_from": 2019, "date_to": 2021,
            "use_llm": True, "intent_summary": "quero por sexo",
        })
        candidates = agente.propose_actions()
        with patch(
        ...
        assert selected[0]["dimensao"] == "POR_SEXO"

    def test_llm_failure_falls_back_to_score(self, agente):
        agente.update_working_memory({
            "analysis_id": "a1", "date_from": 2019, "date_to": 2021,
            "use_llm": True, "intent_summary": "por faixa etária",
        })
        candidates = agente.propose_actions()
...
class TestConsultarIndicadores:
    def test_query_calls_neo4j_with_chosen_dimensao(self, agente, neo4j_client):
        neo4j_client.get_indicadores_por_sistema.return_value = [
            {"tipo": "dengue", "ano": 2020, "valor": 30.0, "dimensao_valor": "0-9"},
        ]
    ...
    def test_no_despesas_key_in_result(self, agente, neo4j_client):
        """Diferente dos agentes legados, AgenteSINAN não consulta
        despesas — isso é responsabilidade de AgenteOrcamentoSubfuncao."""
        neo4j_client.get_indicadores_por_sistema.return_value = []
        result = agente.query("a1", 2019, 2021, use_llm=False)
